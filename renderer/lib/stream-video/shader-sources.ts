...
export const HDR_P010_FRAGMENT_SHADER_SOURCE = `#version 300 es
precision highp float;
precision highp usampler2D;

in vec2 v_texCoord;
uniform usampler2D u_texY;
uniform usampler2D u_texUV;
out vec4 outColor;
...
void main() {
  ivec2 ySize = textureSize(u_texY, 0);
  ivec2 uvSize = textureSize(u_texUV, 0);
  ivec2 yCoord = min(ivec2(v_texCoord * vec2(ySize)), ySize - ivec2(1));
  ivec2 uvCoord = min(ivec2(v_texCoord * vec2(uvSize)), uvSize - ivec2(1));

  float yRaw = float(texelFetch(u_texY, yCoord, 0).r & 1023u);
  uvec2 uvRaw = texelFetch(u_texUV, uvCoord, 0).rg & uvec2(1023u);

  float y = clamp((yRaw - 64.0) / 876.0, 0.0, 1.0);
  float cb = clamp((float(uvRaw.r) - 512.0) / 896.0, -0.5, 0.5);
  float cr = clamp((float(uvRaw.g) - 512.0) / 896.0, -0.5, 0.5);
...
export const FSR_VERTEX_SHADER_SOURCE = `#version 300 es
in vec2 a_position;
in vec2 a_texCoord;
out vec2 v_texCoord;
